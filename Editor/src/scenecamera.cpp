...
namespace Editor {
	...
	SceneCamera::SceneCamera() {
		...
		SceneCamera::load(Core::instance->fileSystem->getDBPath() + "\\scenecamera.xml");
	...
	void SceneCamera::init() {
		...
		SceneCamera::createFBO(Editor::instance->menu->sceneRegion.x, Editor::instance->menu->sceneRegion.y);
	}
	...
	void SceneCamera::controlMouse(float dt) {
		...
		GlfwContext* glfwContext = Core::instance->glfwContext;
		...
		float scenePosX = Editor::instance->menu->scenePos.x;
		float scenePosY = Editor::instance->menu->scenePos.y;
		float sceneRegionX = Editor::instance->menu->sceneRegion.x;
		float sceneRegionY = Editor::instance->menu->sceneRegion.y;
	...
	void SceneCamera::createFBO(int sizeX, int sizeY) {
		...
		Core::instance->glewContext->createFrameBuffer(FBO, RBO, textureBuffer, 1920, 1080); 
	}

	void SceneCamera::recreateFBO(int sizeX, int sizeY) {

		Core::instance->glewContext->recreateFrameBuffer(FBO, textureBuffer, RBO, sizeX, sizeY);
		SceneCamera::updateProjectionMatrix(sizeX, sizeY);
	}

	void SceneCamera::updateProjectionMatrix(int sizeX, int sizeY) {

		aspectRatio = Editor::instance->menu->sceneRegion.x / Editor::instance->menu->sceneRegion.y;
	...
	bool SceneCamera::save(std::string path) {
		...
		std::ofstream file_stored(path); //  + "\\scenecamera.xml"
		file_stored << doc;
		file_stored.close();
		doc.clear();
		return true;
