...
namespace Fury {
	...
	void Terrain::loadTerrainHeightmapOnInit(glm::vec3 camPos, int clipmapLevel) {
		...
		heightData = new unsigned char* [clipmapLevel];
		for (int i = 0; i < clipmapLevel; i++)
			heightData[i] = new unsigned char[MEM_TILE_ONE_SIDE * MEM_TILE_ONE_SIDE * TILE_SIZE * TILE_SIZE * 2];

		for (int level = 0; level < clipmapLevel; level++)
			Terrain::loadHeightmapAtLevel(level,camPos, heightData[level]);

		Terrain::createHeightMapTextureArray(heightData);
		Terrain::deleteHeightmapArray(heightData);
	}
	...
	void Terrain::loadHeightmapAtLevel(int level, glm::vec3 camPos, unsigned char* heightData) {
		...
		for (int i = 0; i < MEM_TILE_ONE_SIDE; i++) {
			...
			for (int j = 0; j < MEM_TILE_ONE_SIDE; j++) {
				...
				std::thread th(&Terrain::loadFromDiscAndWriteGPUBufferAsync, this, level, TILE_SIZE * MEM_TILE_ONE_SIDE, glm::ivec2(startX, tileStart.y), border, heightData);
	...
	void Terrain::streamTerrain(glm::vec3 newCamPos, int clipmapLevel) {
		...
		for (int level = 0; level < clipmapLevel; level++) {
			...
			if (tileDelta.x >= 4 || tileDelta.y >= 4 || tileDelta.x <= -4 || tileDelta.y <= -4) {
		...
		if (tileDelta.x > 0) {
			...
			for (int x = 0; x < tileDelta.x; x++) {
				...
				for (int z = 0; z < MEM_TILE_ONE_SIDE; z++) {
					...
					std::thread th(&Terrain::loadFromDiscAndWriteGPUBufferAsync, this, level, TILE_SIZE, glm::ivec2(old_tileStart.x, startY), glm::ivec2(0, old_border.y), heightData);
					pool.push_back(std::move(th));
		...
		if (tileDelta.y > 0) {
			...
			for (int z = 0; z < tileDelta.y; z++) {
				...
				for (int x = 0; x < MEM_TILE_ONE_SIDE; x++) {
					...
					std::thread th(&Terrain::loadFromDiscAndWriteGPUBufferAsync, this, level, MEM_TILE_ONE_SIDE * TILE_SIZE, glm::ivec2(startX, old_tileStart.y), glm::ivec2(old_border.x, 0), heightData);
					pool.push_back(std::move(th));
	...
	void Terrain::loadFromDiscAndWriteGPUBufferAsync(int level, int texWidth, glm::ivec2 tileStart, glm::ivec2 border, unsigned char* heightData) {

		unsigned char* chunkHeightData = loadTerrainChunkFromDisc(level, tileStart);
		Terrain::writeHeightDataToGPUBuffer(border, texWidth, heightData, chunkHeightData);
	...
	void Terrain::writeHeightDataToGPUBuffer(glm::ivec2 index, int texWidth, unsigned char* heightMap, unsigned char* chunk) {
